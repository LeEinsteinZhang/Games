class Connect:
    ...
    def set_char(self, col, row):
        pos = col + row * self.WIDTH
        self.board[pos] = self.current_player

    def connect_drop_piece(self, col):
        ...
        if (current_player == 2):
            next_player = 1
        top_item = self.get_char(col, 0)
        if (top_item != 0):
            return False
        else:
            for j in range(self.HEIGHT - 1, -1, -1):
                current_player = self.get_char(col, j)
                if (current_player == 0):
                    self.set_char(col, j)
    ...
    def connect_has_won(self):
        # row check
        for i in range(self.HEIGHT):
            row_count = 1
            row_item = self.get_char(0, i)
            for j in range(1, self.WIDTH):
                current_item = self.get_char(j, i)
        ...
        for i in range(self.HEIGHT):
            for j in range(self.WIDTH):
                up_dig_count = 1
                up_dig = self.get_char(j, i)
                dn_dig_count = 1
                dn_dig = self.get_char(j, i)
                for k in range(1, self.WINLENGTH):
                    # up
                    if i - k >= 0 and j + k < self.WIDTH:
                        current_item = self.get_char(j + k, i - k)
    ...
    def connect_has_finished(self):
        if (self.connect_has_won() != 0):
            return True
        top_count = 0
        for j in range(0, self.WIDTH):
            top_item = self.get_char(j, 0)
            if (top_item != 0):
                top_count += 1
